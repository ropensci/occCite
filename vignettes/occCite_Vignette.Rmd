...
<!-- CM: Concerning to get a warning on the first call. i am pretty sure that cynaroides should have no name issues -->
...
<!-- CM: When I compiled this, it got no results from GBIF. pretty sure that's an error -->
...
```
<!-- CM: I got this error, but it still compiled. presumably figuring out why gbif has no king protea would fix this. think there's a chance its hidden because people would pick it? its not that rare... Protea repens would not have that issue, if you want to stay south african.: -->
<!-- ## Error in occurrenceCountGBIF[[i]] <- nrow(x@occResults[[i]]$GBIF$OccurrenceTable): replacement has length zero -->
