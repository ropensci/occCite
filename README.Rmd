...
```{r summary figures overall, eval=T, message=FALSE, warning=FALSE, paged.print=FALSE}
par(mfrow = c(1,3))
...
We can also generate plots for each species in the `occCiteData` object individually.

```{r summary figures by species, eval=T, message=FALSE, warning=FALSE, paged.print=FALSE}
par(mfrow = c(2,3))
sumFig.occCite(myPhyOccCiteObject, 
               bySpecies = T, 
               plotTypes = c("yearHistogram", "source", "aggregator"))
